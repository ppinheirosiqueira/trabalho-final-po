...
pMax = 10          # Numero maximo de mamógrafos disponíveis para alocação
...
bigM = 10000       # Número inteiro positivo que assume no máximo o valor R, em km

auxDados = criar_dados(N,cap,R)
...
model.setObjective(
    gp.quicksum(
        ...
        gp.quicksum(d_ij[i, j] * x[i, j] for i in N for j in S_i[i]) / (N * bigM) #  minimizar a distância total entre os locais indicados a sediar mamógrafos e os que são atendidos por esses equipamentos
    ), GRB.MAXIMIZE
)
...
#         ) - (p * cap * txViab) - ( 
#         gp.quicksum(d_ij[i, j] * t[i, j] for i in N for j in S_i[i]) / (N * bigM)
#     ), GRB.MAXIMIZE
# )
...
for j in N:
    model.addConstr(gp.quicksum(x[i, j] for i in S_i[j]) <= 1, f"R1_{j}")

# Restrição 2: Somatório de y_i = p
model.addConstr(gp.quicksum(y[i] for i in N) == p, "R2")

# Restrição 3: Demanda * x_ij <= Capacidade * y_i
for i in N:
    model.addConstr(gp.quicksum(dem_i[j] * x[i, j] for j in S_i[i]) <= cap * y[i], f"R3_{i}")

# Restrição 4: y_i >= nEquipExist_i (existência de equipamentos)
for i in N:
    model.addConstr(y[i] >= nEquipExist_i[i], f"R4_{i}")

# Restrição 5: y_i >= 1 se infra = 1 e demReg_i >= txMin_i * cap
for i in N:
    model.addConstr(y[i] >= 1 * (infra_i[i] == 1) * (demReg_i[i] >= txMin_i * cap), f"R5_{i}")

# Restrição 6: y_i == 0 se infra = 0
for i in N:
    model.addConstr(y[i] == 0 * (infra_i[i] == 0), f"R6_{i}")

# Restrição 7: z_i >= y_i / pMax
for i in N:
    model.addConstr(z[i] >= y[i] / pMax, f"R7_{i}")

# Restrição 8: x_ii = z_i para todas as i
for i in N:
    model.addConstr(x[i, i] == z[i], f"R8_{i}_{i}")

# Restrição 9: z_i >= x_ij para todas as i e j
for i in N:
    for j in N:
        model.addConstr(z[i] >= x[i, j], f"R9_{i}_{j}")
